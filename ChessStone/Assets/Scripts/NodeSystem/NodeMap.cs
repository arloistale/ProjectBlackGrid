...
public class NodeMap : Singleton<NodeMap>
{
	...
	public void ConfigureObjects() {
		...
		foreach (MapObject o in validObjects) {
			...
			string description = o.GetPropertyAsString("node description");

			MapNode n = NodeBuilder.Instance.BuildNode(domainId, name, description, centerPos);

			// TODO: Fix this
			int stageId = o.GetPropertyAsInt("stage id");
			n.stageId = stageId;
			if(stageId != -1) {
				n.displayQuestDescription = StageManager.Instance.LoadStage(stageId).description;
			}
