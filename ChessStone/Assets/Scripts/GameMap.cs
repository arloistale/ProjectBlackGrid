...
public class GameMap : Singleton<GameMap>
{
	...
	public TileLayer mainLayer { get; private set; }
	public TileSet mainSet { get; private set; }
	...
	public IEnumerator Init() {
		...
		StageData currStage = StageManager.Instance.BuildStage(PlayerData.Instance.currStage);
	...
	private void InitializeTiles() {
		mainLayer = map.GetTileLayer("Tiles");
		mainGrid = mainLayer.Tiles;
		mainSet = map.TileSets[0];

		Vector2 dims = mainSet.WorldDims;
	...
	public void ReplaceTile(int GID, int x, int y) {
		mainLayer.ReplaceTile(GID, x, y, map, new Material(materialDefaultFile) { mainTexture = mainSet.Texture });
		int index = y * mainGraph.Width + x;
		mainGraph.nodes[index].Walkable = mainGrid[x, y].walkable;
	}
	...
	private void InitializePathing() {
		...
		for(int x = 0; x < mainGrid.Width; x++) {
			for(int y = 0; y < mainGrid.Height; y++) {
				...
				node.Walkable = mainGrid[x, y].walkable;
			}
		}
	}
